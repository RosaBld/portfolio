            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 p-10 justify-items-center font-custom">
                {data.sort((a: Project, b: Project) => b.id - a.id).map((project: Project) => (
                    <div key={project.id} className="w-60 justify-center">
                        <div key={project.title} className="flex flex-col items-center bg-gray-50 h-auto w-60 rounded-lg shadow-md transform transition-transform duration-500 hover:scale-105">
                            <Link to={`/Card/${project.id}`}>    
                                ...
                                <div className="p-4 text-left w-60">
                                    <h3 className="font-bold text-xl h-12">{project.title}</h3>
                                    <p className="text-sm pt-4 text-gray-400 h-16">{project.preview}</p>
                            ...
                            <div className="flex flex-row w-60 p-4 justify-between text-xs">
                                <div className="flex flex-row  items-center gap-2">
                                    ...
                                    <a href={project.git}>View Code</a>
                                </div>
                                <div className="flex flex-row  items-center gap-2">
                                ...
                                {project.deploy ? 
                                    <a href={project.deploy}>More</a> : 
                                    <Link to={`/Card/${project.id}`}>More</Link>
                                }
                                </div>
                            </div>
                        </div>
